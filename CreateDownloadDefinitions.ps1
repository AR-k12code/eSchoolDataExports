...
Param(
    ...
    [parameter(Mandatory=$false)][String]$DefinitionName="DBEXP", #By Default the Definition will be called DBEXP. Call it what you want but it must be EXACTLY 5 characters.
...
Import-Csv $PSScriptRoot\info\eSchoolDatabase.csv | Where-Object { $tables -contains $PSItem.tblName } | Group-Object -Property tblName | ForEach-Object {
    ...
    if ($tables_with_years -contains $tblName) {
        if ($sqlspecified) {
            $sql_table += " AND SCHOOL_YEAR = (SELECT CASE WHEN MONTH(GetDate()) > 6 THEN YEAR(GetDate()) + 1 ELSE YEAR(GetDate()) END)"
        } else {
            $sql_table = "WHERE SCHOOL_YEAR = (SELECT CASE WHEN MONTH(GetDate()) > 6 THEN YEAR(GetDate()) + 1 ELSE YEAR(GetDate()) END)"
        }
    }
